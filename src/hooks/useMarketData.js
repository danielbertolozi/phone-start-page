...
async function fetchQuote(symbol, apiKey) {
  ...
  if (data.c === 0 && data.d === 0) throw new Error('No data returned (invalid symbol or market closed?)')
  return data
}
...
export function useMarketData(apiKey, refreshMs = 60000) {
  ...
  const [eurbrl, setEurbrl] = useState({ loading: true, error: null, data: null })
  ...
  const fetchAll = useCallback(async () => {
    ...
    // Fallback: try "SAP.F" (Frankfurt) which is equivalent
    fetchQuote('SAP.XETRA', apiKey)
      .then(data => setSap({ loading: false, error: null, data }))
      .catch(() =>
        // Try Frankfurt (equivalent to XETRA for SAP)
        fetchQuote('SAP.F', apiKey)
          .then(data => setSap({ loading: false, error: null, data }))
          .catch(err => setSap({ loading: false, error: err.message, data: null }))
      )

    fetchForex('OANDA:EUR_BRL', apiKey)
      .then(data => setEurbrl({ loading: false, error: null, data }))
      .catch(err => setEurbrl({ loading: false, error: err.message, data: null }))
  }, [apiKey])
